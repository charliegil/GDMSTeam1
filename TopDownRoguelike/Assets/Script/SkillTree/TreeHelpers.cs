...
using System.Linq;
    ...
    public static class TreeHelpers    
    {
        ...
        public static void CalculateNodePositions(treeNode rootNode)
        {
        ...
        private static void InitializeNodes(treeNode node, int depth)
        {
            node.X = -1;
            node.Y = depth;
            node.Mod = 0;
        ...
        private static void CalculateFinalPositions(treeNode node, float modSum)
        {
            node.X += modSum;
            modSum += node.Mod;
        ...
        private static void CalculateInitialX(treeNode node)
        {
            foreach (var child in node.Children)
        ...
        private static void CheckForConflicts(treeNode node)
        {
        ...
        private static void CenterNodesBetween(treeNode leftNode, treeNode rightNode)
        {
        ...
        private static void CheckAllChildrenOnScreen(treeNode node)
        {
            var nodeContour = new Dictionary<int, float>();
            GetLeftContour(node, 0, ref nodeContour);
        ...
        private static void GetLeftContour(treeNode node, float modSum, ref Dictionary<int, float> values)
        {
        ...
        private static void GetRightContour(treeNode node, float modSum, ref Dictionary<int, float> values)
        {
            if (!values.ContainsKey(node.Y))
                values.Add(node.Y, node.X + modSum);
            else
